...
/**
 * Default policy. All policy decisions are based off of response data received from the licensing service. Specifically, the licensing server sends the following information: response validity period, error retry period, and error retry count.
 * <p>
...
public class ServerManagedPolicy implements Policy {
	...
	public ServerManagedPolicy(Context context, Obfuscator obfuscator) {
		...
		mLastResponse = Integer.parseInt(mPreferences.getString(PREF_LAST_RESPONSE, Integer.toString(Policy.RETRY)));
	 ...
	 * <ul>
	 * <li>VT: the timestamp that the client should consider the response valid until
	...
	/**
	 * Set the last license response received from the server and add to preferences. You must manually call PreferenceObfuscator.commit() to commit these changes to disk.
	 * 
	 * @param l
	 *            the response
	 */
	...
	/**
	 * Set the current retry count and add to preferences. You must manually call PreferenceObfuscator.commit() to commit these changes to disk.
	 * 
	 * @param c
	 *            the new retry count
	 */
	...
	/**
	 * Set the last validity timestamp (VT) received from the server and add to preferences. You must manually call PreferenceObfuscator.commit() to commit these changes to disk.
	 * 
	...
	/**
	 * Set the retry until timestamp (GT) received from the server and add to preferences. You must manually call PreferenceObfuscator.commit() to commit these changes to disk.
	 * 
	 * @param retryUntil
	 ...
	 * <li>a RETRY response was received in the last minute, and we are under the RETRY count or in the RETRY period.
	 * </ol>
	 */
	public boolean allowAccess() {
		...
		} else if (mLastResponse == Policy.RETRY && ts < mLastResponseTime + MILLIS_PER_MINUTE) {
